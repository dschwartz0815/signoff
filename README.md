...
| [`packages/signoff-code`](./packages/signoff-code) | Python | First verifier pack — five verifiers for Python code changes ([docs](./docs/packs/signoff-code.md)). |
...
**Phase 1 in flight — first real verification landed.** All infrastructure (`signoff-core`, `signoff-mcp`, `signoff-http`, `signoff-judge`, `signoff-runtime-docker`) plus the first verifier pack, `signoff-code`, are live. An agent can now call `request_signoff` against a `code_change` deliverable and get real answers back: tests run, types are checked, lint is clean, imports work, and an LLM judge cross-checks the diff against the stated intent. See [`docs/packs/signoff-code.md`](./docs/packs/signoff-code.md) for the full pack surface. Next: polish and additional packs per [`CLAUDE.md`](./CLAUDE.md) §14.
